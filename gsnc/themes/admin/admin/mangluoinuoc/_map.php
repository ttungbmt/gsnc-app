...
<?php
    $lat = $model->geom[0][0][1];
    $lng = $model->geom[0][0][0];
?>
<script>
    $(function(){
        mbUrl = 'https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoibG9pdG4xNDgiLCJhIjoiY2pmdWl1b3FrMDI1ejMzbGF3YTVzNjdsaiJ9.UdxaQVcFv9BY0b0EXmZ4Jg';
        ...
        if(!flag) {
            let coordinates = <?=json_encode($model->geom[0])?>;
            let myLines = [{
                "type": "LineString",
        ...
        function addMaker (lat, lng) {
            ...
            $('#mangluoinuoc-lat').val(lat);
            $('#mangluoinuoc-lng').val(lng);
            ...
            marker.on('dragend', function ondragend() {
                ...
                $('#mangluoinuoc-lat').val(m.lat);
                $('#mangluoinuoc-lng').val(m.lng);
            });
        ...
        function removeMaker() {
            $('#mangluoinuoc-lat').val(null);
            $('#mangluoinuoc-lng').val(null);
            map.removeLayer(marker)
        }
                ...
                if (status === google.maps.GeocoderStatus.OK) {
                    if (results[1]) {
                        getPlaceDetail(results[1].place_id, function(detail) {
                            $('#mangluoinuoc-diachi').val(detail.formatted_address);
                        });
                      } else {
