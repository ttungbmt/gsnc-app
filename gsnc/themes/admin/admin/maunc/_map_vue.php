...
<link rel="stylesheet"
      href=<?= '/libs/bower/leaflet-google-places-autocomplete/src/css/leaflet-gplaces-autocomplete.css' ?>/>

<style>
    .leaflet-touch .leaflet-left .leaflet-bar { border: none; }

    #predictions { position: absolute; bottom: 0; left: 0; max-height: 400px; width: 320px; z-index: 1000; transition: .3s ease; }
    ...
    $(function () {
        let vueMap = new Vue({
            ...
            mounted(){
                ...
                if (!flag) {
                    this.latlng = [<?=$model->lat ? $model->lat : 0 ?>, <?=$model->lng ? $model->lng : 0 ?>]
                    this.addMaker(...this.latlng);
                    map.setView(this.latlng, 18);
                }
                        ...
                        predictions.forEach(function (prediction) {
                            getPlaceDetail(prediction.place_id, function (detail) {
                                ...
                                li.addEventListener('click', function () {
                                    ...
                                    /**
                                     * Hidden Prediction table
                                     **/
                                    let btn_control = $(".toggle-result span");
                                    if (btn_control.hasClass('glyphicon-triangle-left')) {
                                        $('#predictions').css('width', 0);
                                        btn_control.removeClass('glyphicon-triangle-left');
                                        btn_control.addClass('glyphicon-triangle-right');
                                    }
                                }, false);
                            });
                ...
                $(".close-result span").click(function () {
                    $('#predictions').css('display', 'none');
                })
            },
            methods: {
                addControls(){
                    ...
                    let mapboxLayer = L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoibG9pdG4xNDgiLCJhIjoiY2pmdWl1b3FrMDI1ejMzbGF3YTVzNjdsaiJ9.UdxaQVcFv9BY0b0EXmZ4Jg', {id: 'mapbox.streets'});
                    ...
                    let layersCtrl = L.control.layers(baseLayer).addTo(this.$map)

                    let zoomCtrl = L.control.zoom({
                        position: 'bottomright'
                    }).addTo(this.$map)

                    let googleCtrl = new L.Control.GPlaceAutocomplete({
                        position: 'topleft',
                        callback: (place) => {
                            let loc = place.geometry.location;
                            this.$map.setView([loc.lat(), loc.lng()], 18);
                            this.addMaker(loc.lat(), loc.lng());
                        }
                    }).addTo(this.$map)
